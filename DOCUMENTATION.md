...
2. O Backend hasheia com `Bcrypt` (uma criptografia irremovível) e acha ele no SQLite/PostgreSQL.
...
### Funcionalidades de Segurança Adicionais Implementadas:
* **Código de Convite Corporativo (`INVITE_CODE`)**: Cadastro bloqueado para curiosos online. Apenas quem tem a chave cadastrada na Nuvem pode se registrar.
* **Protocolos Únicos**: Ao salvar cotações, o sistema gera UUIDs rastreáveis e visíveis no UI/PDF (ex: `24022026-X8Y9Z0`).
* **Deleção de Histórico**: Um endpoint protegido rigorosamente no backend só permite que usuários com a flag `is_admin = True` enxerguem o botão de deletar e modifiquem o banco de dados.
